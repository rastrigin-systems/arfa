...
The `pr-reviewer` agent is a specialized agent that handles the complete PR lifecycle:
...
7. Update GitHub Project status to "Done"
8. Cleanup worktree (if applicable)
...
Agent configuration should be stored in:
```
...
Agent:
1. Gets PR info
2. Checks status (all green)
3. Reviews code (looks good)
4. Merges PR
5. Deletes branch
6. Updates issue to Done
...
```
User: "Review and merge PR #21"

Agent:
1. Gets PR info
2. Detects merge conflicts
3. Checks out branch
4. Resolves conflicts (non-overlapping changes)
5. Commits and pushes
6. Waits for CI/CD checks
7. Merges PR
8. Cleans up
...
```
User: "Review and merge PR #22"

Agent:
1. Gets PR info
2. Checks status (tests failing)
3. Comments on PR with failure details
4. STOPS - does not merge
5. Reports to user: "PR #22 has failing tests, please fix before merging"
```
...
- Place this file at: `~/.claude/agents/pr-reviewer.md`
- Agent will automatically follow this workflow when invoked
- Use command: "Review and merge PR #<number>"
- Agent will ask for clarification on complex conflicts
